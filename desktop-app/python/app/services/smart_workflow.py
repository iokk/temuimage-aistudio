...
from app.services.smart_generation import SMART_TYPES, SmartGenerationError, SmartGenerator
...
def create_smart_workflow(
    ...
    *,
    ...
    try:
        ...
        if not provider:
            raise SmartGenerationError("Provider 不存在。")
        normalized_types = {
            key: int(value)
            for key, value in selected_types.items()
            if key in SMART_TYPES and int(value) > 0
        }
        if not normalized_types:
            raise SmartGenerationError("至少选择一种组图类型。")

        total_images = sum(normalized_types.values())
        artifact_dir = Path(runtime.files_dir) / "projects" / f"smart-{uuid.uuid4().hex[:12]}"
        artifact_dir.mkdir(parents=True, exist_ok=True)
        project = create_project(
            connection,
            project_type="smart_generate",
            summary=f"智能组图 · {product_name} · {total_images}张",
            status="queued",
        ...
        add_task_event(
            ...
            event_type="queued",
            message="智能组图任务已进入队列。",
            detail={"total_images": total_images, "selected_types": normalized_types},
        )
    finally:
        connection.close()

    thread = threading.Thread(
        target=run_smart_task,
        args=(
            app,
            task["id"],
            project["id"],
...
def run_smart_task(
    ...
    try:
        provider = get_provider(connection, provider_id)
        total_images = sum(selected_types.values())
        update_task(
            connection,
            task_id,
            status="running",
            progress_done=0,
            progress_total=total_images,
            current_step="generating_smart_images",
        ...
        generated_count = 0
        for type_key, count in selected_types.items():
            for index in range(count):
                ...
                type_name = SMART_TYPES[type_key]["name"]
                output_path = output_dir / f"{generated_count + 1:02d}-{type_key}-{index + 1}.png"
                ...
                update_task(
                    ...
                    progress_total=total_images,
                    current_step=f"{type_name}_{generated_count}",
                )
        ...
        add_task_event(
            ...
            message="智能组图完成并已归档。",
            detail={"image_count": generated_count, "selected_types": selected_types},
        )
        ...
        update_task(
            ...
            status="failed",
            progress_done=0,
            progress_total=sum(selected_types.values()),
            current_step="failed",
