...
class TriTitleTests(unittest.TestCase):
    def test_builtin_strategy_omits_legacy_template_context(self):
        prompt = app.build_temu_tri_title_prompt("", "不锈钢保温杯 500ml")
        self.assertIn("商品图片（如有）", prompt)
        self.assertNotIn("【类目模板参考", prompt)

    def test_legacy_task_template_context_remains_compatible(self):
        prompt = app.build_temu_tri_title_prompt(
            "Legacy strategy for {product_info}", "不锈钢保温杯 500ml"
        )
        self.assertIn("【类目模板参考", prompt)
        self.assertIn("Legacy strategy for 不锈钢保温杯 500ml", prompt)
